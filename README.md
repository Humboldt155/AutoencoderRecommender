...
### Кодирование моделей по месяцам до и после покупки
Проблема с полследовательность покупок во времени была решена разделением моделей Адео, и 
созданием копии каждой модели в каждом месяце.
То есть для модели Дрели создается еще 24 модели, существующие в -12, -11 ... 11, 12 месяцах от   
текущей даты. Обучающая модель в таком случае не знает о том, что модель -20125, 125 и +30135 это   
один и тот же товар, купленный за 2 месяца, сейчас и через 3 месяца.   
А раз так, то если некий проект клиента включает покупку одной и той же товарной категории в прошлом   
и в будущем, то мы сможем закодировать историю покупок, а затем декодировать прогноз более точно и полно.   
